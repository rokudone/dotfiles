...
`$ARGUMENTS` に文字列がある場合のみ `--project <文字列>` を付与する。それ以外のオプションはない。

```bash
# 引数なし → 全期間・全プロジェクト
python ~/.claude/skills/prompt-review/scripts/collect.py > /tmp/prompt-review-data.json

# 文字列あり → プロジェクトフィルタ
python ~/.claude/skills/prompt-review/scripts/collect.py --project yonshogen > /tmp/prompt-review-data.json
```

**重要**: スクリプトのパスは絶対パス `~/.claude/skills/prompt-review/scripts/collect.py` を使うこと。
