...
for i = 1, 10, 1 do
	local space = sbar.add("space", "space." .. i, {
		...
		label = {
			...
			padding_left = 3,
			color = colors_spaces[i],
			font = "sketchybar-app-font-bg:Regular:23.0",
			y_offset = -2,
		},
		padding_right = 4,
		padding_left = 4,
...
local aerospace_workspace_watcher = sbar.add("item", "aerospace_workspace_watcher", {
	drawing = false,
	updates = true,
	script = "$CONFIG_DIR/plugins/aerospace_workspace_number.sh",
	update_freq = 1,
})
...
space_window_observer:subscribe("space_windows_change", function(env)
	local icon_line = ""
	local fallback_icon = app_icons["default"] or ":default:"
	for app, count in pairs(env.INFO.apps) do
		local lookup = app_icons[app] or fallback_icon
		icon_line = icon_line .. utf8.char(0x202F) .. lookup
	end
